...
def install_dependencies():
    ...
    if os.path.exists(requirements_path):
        ...
        for package in packages:
            try:
                __import__(package.lower() if package.lower() != "pillow" else "pil")
            except ImportError:
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"screenshot_ocr_{datetime.now().strftime('%Y%m%d')}.log")
...
if API_KEY:
    logger.info(f"API密钥已设置，前10位: {API_KEY[:10] + '...' if len(API_KEY) > 10 else API_KEY}")
else:
...
class ScreenshotTool:
    ...
    def call_ocr_api(self, img_base64):
        ...
        if not API_KEY:
            ...
            print("未设置 ALIYUN_DASHSCOPE_API_KEY，跳过 OCR 调用。请在环境变量中配置。")
            return
        try:
            ...
            # 设置OpenAI API密钥和基础URL（使用传统方式）
            openai.api_key = API_KEY
            openai.api_base = BASE_URL
            
            logger.info("开始发送API请求...")
            # 使用传统ChatCompletion.create方法
            ...
            try:
                logger.info("使用JSON字符串解析API响应")
                
                # 将API响应转换为字符串然后解析为JSON
                ...
                logger.info(f"JSON结构: {list(response_dict.keys()) if isinstance(response_dict, dict) else 'Not a dict'}")
                
                # 标准OpenAI API响应结构解析
                if 'choices' in response_dict and len(response_dict['choices']) > 0:
                    choice = response_dict['choices'][0]
                    logger.info(f"找到choices，第一个choice结构: {list(choice.keys()) if isinstance(choice, dict) else 'Not a dict'}")
                    
                    if 'message' in choice and isinstance(choice['message'], dict) and 'content' in choice['message']:
                        actual_result = choice['message']['content'].strip()
                        logger.info(f"通过JSON解析成功提取识别结果，长度: {len(actual_result)} 字符")
                        ...
                        if 'message' in choice:
                            logger.info(f"choice['message']类型: {type(choice['message'])}")
            ...
            # JSON解析失败的最终回退
            logger.error("JSON解析失败，无法提取识别结果")
            ...
            print(f"调用API发生错误: {e}")
            # 提供更详细的错误信息
            print(f"API Key前10位: {API_KEY[:10] + '...' if API_KEY and len(API_KEY) > 10 else '未设置'}")
...
def create_startup_shortcut():
    ...
    try:
        ...
        working_dir = os.path.dirname(script_path) if os.path.dirname(script_path) else startup_folder
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='屏幕截图OCR工具 - 自动识别截图中的文字')
    parser.add_argument('--enable-autostart', action='store_true', help='启用开机自动启动')
    parser.add_argument('--disable-autostart', action='store_true', help='禁用开机自动启动')
    parser.add_argument('--check-autostart', action='store_true', help='检查开机自动启动状态')
    parser.add_argument('--no-hide', action='store_true', help='不隐藏控制台窗口')
    parser.add_argument('--no-delay', action='store_true', help='不延迟启动（默认延迟1秒）')
